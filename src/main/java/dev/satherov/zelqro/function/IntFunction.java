...
///
/// Represents a function that accepts a `int` argument and produces a result.
///
...
public interface IntFunction<R extends @Nullable Object> {
    ...
    ///
    default <V> Function<V, R> compose(ToIntFunction<? super V> before) {
        return value -> this.apply(before.applyAsInt(value));
    }
}
