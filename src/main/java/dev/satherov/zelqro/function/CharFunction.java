...
///
/// Represents a function that accepts a `char` argument and produces a result.
///
...
public interface CharFunction<R extends @Nullable Object> {
    ...
    ///
    default <V> Function<V, R> compose(ToCharFunction<? super V> before) {
        return value -> this.apply(before.applyAsChar(value));
    }
}
